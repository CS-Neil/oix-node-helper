...
节点数量由两层过滤决定，设置页的「筛选与高级选项」里都能调：

| 层 | 设置项 | 作用位置 | 说明 |
|---|---|---|---|
| 上游 | 订阅过滤参数 | OixCloud 服务端 | 拼接到 managed subscription 地址后面，决定官方**返回**哪些节点 |
| 本地 | 包含正则 / 排除正则 | 助手内部 | 对已经拿到的节点再按名称筛一遍 |

如果官网显示的节点数明显多于助手拿到的数量，差额通常来自上游这一层，而不是本地正则。

订阅过滤参数写成 `&key=value` 形式，多个参数直接相连：

```text
&mode=premium&love=1
```

粘贴 `mode=premium` 或 `?mode=premium` 也可以，保存时会自动补成上面的规范形式。

留空表示使用套餐默认值。注意留空**不等于**把参数清空：核心把空参数理解成「这个账户没有参数」，会连套餐默认值一起丢掉，所以助手在留空时会替你回填套餐默认值。

保存后助手会重启核心并重新拉取订阅，设置页下方的「核心当前生效」显示核心实际使用的参数。核心会自动补回它保留的键（例如 `tfo`），所以生效值通常比填进去的多几项，这是正常的。

当前生效值也可以直接查：

```bash
curl -s --noproxy '*' http://127.0.0.1:6172/health
```

放宽过滤让上游返回更多节点时，注意「最大节点数」默认是 100：超出的部分会被直接截断。需要更多节点就同时调大「最大节点数」，并确认节点端口范围（起始端口 + 最大节点数）不与 Provider 端口或其它程序冲突。
...
| 路径 | 用途 |
|---|---|
| `GET /health` | 阶段、核心状态、节点数、上次成功时间、错误、核心资源指标和生效的订阅参数 |
